...
export async function measureDemo(page: Page) {
  return page.evaluate(
    ({ surfaceSelectors, contrastSelectors }) => {
      ...
      for (const element of [
        ...
        document.body,
        document.querySelector('.demo-shell'),
      ]) {
      ...
      for (const element of document.querySelectorAll('.demo-shell *')) {
      ...
      const targets = [...document.querySelectorAll('.demo-shell button, .demo-shell a[href]')]
        .filter(visible)
        .map((element) => {
          const rect = element.getBoundingClientRect();
          const size = Math.min(rect.width, rect.height);
          if (size < 44)
            violations.push({ kind: 'target', element: label(element), measured: size });
