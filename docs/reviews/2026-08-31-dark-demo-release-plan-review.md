...
The user clarified that triggering the existing Vercel automatic deployment is the purpose of merging. This automatic effect is explicitly authorized; no separate manual deployment or configuration change is requested. The 99/100 merge-only review remains applicable, and an observed GitHub deployment status may be reported without claiming an unverified success.
