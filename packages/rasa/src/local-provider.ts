...
const templates = {
  1: '문제에서 무엇이 계속 유지되는지 먼저 찾아보자.',
  2: '실험 전과 후의 운동 상태를 각각 비교해 보자.',
  3: '원인, 변한 것, 유지된 것을 차례로 설명해 보자.',
} as const;

export function buildLocalHintContent(hintLevel: 1 | 2 | 3): string {
  return templates[hintLevel];
}
...
export class LocalRasaProvider implements RasaHintProvider {
  ...
  generateHint(input: RasaProviderInput, signal: AbortSignal) {
    ...
    const content = buildLocalHintContent(input.hintLevel);
