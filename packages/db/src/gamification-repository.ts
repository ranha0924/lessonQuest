...
export class GamificationRepository {
  ...
  async createCampaign(
    ...
    classIdInput: string,
    inputValue: CreateBossCampaignInput,
    traceIdInput: string,
  ...
  async endCampaign(
    ...
    inputValue: EndBossCampaignInput,
    traceIdInput: string,
  ): Promise<TeacherBossDetail> {
    const actor = actorSchema.parse(actorInput);
    const organizationId = uuidSchema.parse(organizationIdInput);
      ...
      try {
        await this.database.transaction(async (tx) => {
          ...
          const e = event.rows[0],
            c = campaign.rows[0];
          if (e === undefined || c === undefined) throw new Error('MISSING_PROJECTION_SOURCE');
          const payload = json<{ correct?: boolean; attempt?: number }>(e.payload);
          let kind: VerifiedBossOutcome['kind'] | undefined;
          if (e.type === 'QUESTION_ANSWERED' && payload.correct === true && payload.attempt === 1)
            kind = 'ANSWER_CORRECT';
          if (
            e.type === 'ANSWER_RETRIED' &&
          ...
          if (kind !== undefined) {
            ...
            for (const item of contributions)
              await tx.query(
                `INSERT INTO boss_contributions(id,organization_id,campaign_id,student_id,source_event_id,amount,reason) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(organization_id,source_event_id) DO NOTHING`,
                [
  ...
  private async readDetail(
    ...
    if (c === undefined) throw new ResourceNotFoundError();
    const rows = await q.query<{ student_id: string; damage: number; reasons: string[] }>(
      `SELECT student_id,SUM(amount)::int damage,ARRAY_AGG(DISTINCT reason ORDER BY reason) reasons FROM boss_contributions WHERE organization_id=$1 AND campaign_id=$2 GROUP BY student_id ORDER BY student_id`,
      [organizationId, campaignId],
    );
    const health = await q.query<{ pending: number; failed: number }>(
      `SELECT COUNT(*) FILTER(WHERE status IN('PENDING','PROCESSING'))::int pending,COUNT(*) FILTER(WHERE status='FAILED')::int failed FROM boss_projection_jobs WHERE organization_id=$1 AND campaign_id=$2`,
      [organizationId, campaignId],
    );
    return teacherBossDetailSchema.parse({
      campaign: {
        campaignId: c.id,
        title: c.title,
      ...
      projectionHealth: health.rows[0] ?? { pending: 0, failed: 0 },
    });
  }
  private async audit(
    q: Queryable,
    ...
    await q.query(
      ...
      [randomUUID(), traceId, actorId, organizationId, action, resourceId, outcome],
    );
  }
}
